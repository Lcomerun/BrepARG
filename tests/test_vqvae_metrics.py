...
def test_bucket_mse_is_sample_weighted_and_keeps_empty_buckets_visible():
    ...
    assert summary["edge"] == {"samples": 2, "mse": 2.0}
    assert summary["surface_curved_proxy"] == {"samples": 1, "mse": 2.0}
    assert summary["surface_planar_like"] == {"samples": 0, "mse": None}
...
def test_validation_accumulator_uses_all_batches_for_usage_and_bucket_mse():
    ...
    assert summary["reconstruction_mse"]["edge"] == {"samples": 2, "mse": 1.5}
