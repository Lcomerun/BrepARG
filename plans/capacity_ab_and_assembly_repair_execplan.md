...
The face/wire diagnosis milestone, the P0-B stability archive, and the fixed 100-CAD bypass-versus-VQ-4096 measurement are complete and Git-safe. Schema v1/v2 localizes the 16 invalid controls and the archive records all 96 diagnosis attempts. The P0-B archive contains four 100-epoch histories, four TensorBoard event files, compact logs, metrics, manifests, and checkpoint SHA-256/size bindings; the 4.27 GiB checkpoint bytes remain local. On the identical 100-CAD cohort, bypass@60k is 70/100 strict and VQ-4096@60k is 57/100 strict, giving `Delta_q=13 pp`; the historical GT-to-bypass gap is `Delta_r=14 pp`. The registered decision is `CAPACITY_AB_FIRST`, so boundary consistency, sequence regeneration, and AR remain blocked. The plan is not complete until the VQ-8192/RVQ capacity comparison, remaining no-regression repairs, repaired-chain winner measurement, and their Git-safe snapshots are verified.
