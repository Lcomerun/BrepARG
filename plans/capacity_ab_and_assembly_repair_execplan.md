...
- [x] (2026-08-17 16:45 +08:00) Snapshotted the completed guarded `directed_trim` 100-CAD matrix from clean commit `6bfe0c1`: 100 unique attempts, 97 STEP-readable, 88 native-valid, 86 strict-valid, 83 both-valid, two restored CADs (`00016845...`, `00032004...`), and zero regressions. The guard removes the prior `00044862...` regression but still misses the 95/100 gate.
...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root remains healthy: VQ-8192 seed 3 has completed 100 epochs and seed 4 is running. Baseline parity is exact at 84/100. Conservative local topology repair reaches 85/100 with one restoration and zero regressions, guarded directed trim reaches 86/100 with two restorations and zero regressions, and their guarded combination reaches 87/100 with three restorations and zero regressions. These are valid independent partial results, not the 95/100 assembly solution. The plan is not complete until all four capacity runs, unchanged-chain capacity measurements, the remaining independent no-regression repairs, repaired-chain winner measurement, Git-safe snapshots, and remote push are verified.
