...
- [ ] Let the immutable launcher finish all four formal tasks serially (completed: both VQ-8192 seeds and RVQ seed 3 through epoch 99 with formal validation PASS; running: RVQ seed 4).
...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root remains healthy, with the VQ-8192 pair complete and RVQ continuing serially. A short-path post-hardening source at `D:\capv5` is dry-run ready but deliberately not started while the immutable older formal run is still active. Baseline parity is exact at 84/100. Local topology reaches 85/100, guarded directed trim reaches 86/100, and their explicit combination reaches 87/100, all with zero regressions. The corrected local-pcurve-continuity profile independently reaches 86/100 and adds no recovery beyond guarded directed trim; selective curve rescue changes the three curve-fit failures into STEP-invalid rows but adds no strict-valid CAD beyond the 87/100 combination. The new near-vertex repair explains and restores the one non-unit-solid case without regressing any original success: its signed matrix is 95 STEP-readable, 87 native-valid, 85 strict-valid, and 83 both-valid. It remains an independent primitive rather than a promoted chain because it is still ten CADs below 95/100. The new selector preserves strict-valid primary outputs, stores every isolated candidate in a resume-safe ledger, and can accept only a native/strict/both-valid fallback whose topology and bidirectional geometry gate pass. Its two known fallback families have each completed a one-CAD child-worker smoke; the 16-CAD pilot remains the next proof. Geometric-loop and relaxed-gate probes were rejected as non-promoting or too risky. The assembly release gate remains closed at eight CADs below 95/100 when measured against the best explicit no-regression composition, so boundary consistency, sequence regeneration, and AR remain blocked pending a different diagnosed repair or a formal gate decision. The plan is not complete until all four capacity runs, unchanged-chain capacity measurements, the selector and remaining no-regression repairs, repaired-chain winner measurement, Git-safe snapshots, and remote push are verified.
