...
- [ ] Register the separately probed minimal `directed_trim_surface_precision_curve_interpolate` fallback, prove first-eligible persistence ordering in tests, and run it first on the same frozen 16 invalid CADs. Only a pilot that retains the previous six restorations and safely adds `00008763...` may advance to another 100-CAD matrix.
...
- Decision: Evaluate the minimal surface-precision plus interpolation composition as the next selector fallback, without local intersection topology.
  Rationale: The clean isolated probe shows that this exact composition restores `00008763...` and passes the complete unchanged gate. Excluding `local_intersection_topology` keeps the candidate tied to the measured cause and avoids attributing the recovery to an unnecessary post-fit topology mutation. `00032101...` and `00076198...` remain registered negative controls because their OCC-valid outputs fail semantic topology preservation.
