...
- [x] (2026-08-17 21:23 +08:00) Completed all four immutable schema-v2 capacity tasks and ran the formal validator. Each arm/seed has exactly epochs 0--99, the same ordered 60,000/12,000 inventory hashes, `469/469` finite training batches and `94/94` finite validation batches per epoch, and zero non-finite events. RVQ seed 4 resumed after one Windows checkpoint-replace permission error; the resumed attempt remained signature-compatible and passed the same validator.
- [x] (2026-08-17 21:39 +08:00) Completed the unchanged-chain 100-CAD capacity measurement from the validated seed-3 best checkpoints. The registered five-column strict comparison is `GT 84 | bypass@300k 70 | FSQ@300k 49 | bypass@60k 70 | VQ-8192@60k 69`; therefore `Delta_q=1 pp` and `Delta_r=14 pp`. VQ-8192 passes the 5-point capacity gate. RVQ reaches 65/100, has five paired strict wins and nine losses against VQ-8192 (`p=0.42395`), and is rejected given its estimated 36 percent sequence cost. The capacity decision is `VQ_8192_DIRECT_WIN`; the numeric boundary-loss trigger remains true but execution stays held on the independent assembly-chain gate and review.
...
- [x] (2026-08-18 00:45 +08:00) Committed the reviewed high-precision selector as `283734d`, completed its clean 16-invalid pilot, and then completed the fixed 100-CAD matrix. The pilot restored the exact registered six CADs and the full matrix reached 97 STEP-readable, 90 native-valid, 90 strict-valid, and 87 both-valid. All original 84 strict controls were retained, the unchanged schema-v2 gate passed for every selected fallback, and worker/protocol failures remained zero. Git-safe snapshots are in `reports/assembly_selector_surface_precision_invalid16_20260818/` and `reports/assembly_selector_surface_precision_100cad_20260818/`.
- [x] (2026-08-18 01:38 +08:00) Registered the minimal `directed_trim_surface_precision_curve_interpolate` fallback, proved first-eligible persistence ordering in the focused tests, and committed it as `ddf5ac6`. The clean 16-invalid pilot restored exactly the prior six plus `00008763...` at 7/16 strict and both-valid with 62 candidate attempts and zero worker/protocol failures. The conditional fixed 100-CAD matrix then reached 97 STEP-readable, 90 native-valid, 91 strict-valid, and 88 both-valid with 146 candidates, all original 84 controls retained, and zero regressions. Both selector protocols passed; the 95/100 release gate remains closed by four CADs. Git-safe snapshots are in `reports/assembly_selector_surface_precision_interpolate_invalid16_20260818/` and `reports/assembly_selector_surface_precision_interpolate_100cad_20260818/`.
...
- Observation: Surface precision and interpolation together can remove a real wire self-intersection without requiring a topology-repair pass, but OCC validity alone still cannot establish semantic preservation.
  Evidence: The isolated no-local-topology probe makes `00008763...` native/strict/both-valid and passes schema v2 with 23 faces, 62 edges, 40 vertices, equal incidence signatures, bbox relative delta `0.002883`, and edge-length relative delta `0.000133`. The same probe makes `00032101...` and `00076198...` look valid to OCC while changing vertex or edge incidence; the unchanged gate rejects both.
...
- Decision: Admit high-precision surface fitting only as the selector's last fallback, without curve rescue, and only through the unchanged schema-v2 geometry/topology gate.
  Rationale: The isolated no-confound probe recovers the pre-registered missing `00051587...` while preserving topology, whereas the same treatment on `00032101...` changes vertex incidence and is correctly rejected. Failure-only routing preserves primary successes and avoids treating improved validity alone as proof of semantic equivalence.
  Date/Author: 2026-08-17 / Codex.

- Decision: Retain the formally validated minimal surface-precision plus interpolation composition as the selector's last fallback, without local intersection topology.
  Rationale: The clean pilot and fixed 100-CAD matrix confirm that this exact composition restores only `00008763...`, passes the complete unchanged gate, and introduces no strict regression. Excluding `local_intersection_topology` keeps the recovery tied to the measured cause and avoids an unnecessary post-fit topology mutation. `00032101...` and `00076198...` remain negative controls because their OCC-valid outputs fail semantic topology preservation.
...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root is formally complete: all four 100-epoch tasks validate with identical 60k/12k inventories and zero non-finite events. Its Git-safe snapshot preserves all four histories, logs, checkpoint hashes, and five TensorBoard segments, including the legitimate RVQ seed-4 resume segment. The unchanged-chain fixed-cohort result is conclusive: bypass, VQ-8192, and RVQ reach 70, 69, and 65 strict-valid CADs respectively. VQ-8192 passes the capacity gate with a one-point bypass gap and is selected; RVQ's lower MSE does not translate into better validity and its paired result cannot justify longer sequences. Baseline parity is exact at 84/100. The failure-triggered selector now composes only failure-local, independently gated candidates. Its latest completed fixed-cohort result is 91/100 strict, 88/100 both-valid, and zero regressions after the minimal surface-precision plus interpolation fallback safely restores `00008763...`. The selector restores seven original failures and continues to reject `00032101...` and `00076198...` because their OCC-valid candidates change semantic topology. This is the current formal no-regression maximum, four below the immutable 95/100 assembly gate. Boundary consistency, repaired-chain representation measurement, sequence regeneration, and AR therefore remain blocked. Capacity selection and its Git-safe evidence are complete; the plan remains open only for the remaining assembly-chain acceptance path, repaired-chain winner measurement, and downstream release decision.
...
The failure-triggered selector is a separate local runner, not a new global construction profile. `tools/run_assembly_repair_selector.py` always executes the guarded primary in a child process. A primary strict-valid result is copied to the final selector row immediately and no fallback is started. For a strict-invalid primary, it attempts near-vertex reconciliation, curve interpolation, and finally high-precision surface fitting without curve rescue. Each fallback must have a saved STEP, status `both_valid`, native validity, strict validity, conjunction validity, and an accepted gate. `tools/assembly_selector_geometry.py` measures the gate in the child: it compares the post-joint-placement input with the actual STEP using unique face/edge/vertex/solid counts, incidence multisets, strict validity-component counts, optimal trimmed-shape bounds, total edge length, input-to-candidate curve projection, and candidate-to-input polyline distance. The parent process does not import OCC for this stage.
...
Completed capacity decision:

    bypass@60k strict:      70/100
    VQ-8192@60k strict:     69/100
    RVQ-2x4096@60k strict:  65/100
    VQ-8192 Delta_q:         1 percentage point
    RVQ-only / VQ-only:      5 / 9
...
Revision note 2026-08-18 01:02 +08:00: Recorded the completed clean 16-invalid and fixed 100-CAD high-precision selector matrices at 90/100 strict, 87/100 both-valid, and zero regressions, together with their Git-safe report locations. Registered the separately measured minimal surface-precision plus interpolation candidate for `00008763...`; it must pass the same clean pilot and formal matrix before its expected 91/100 result may be counted.

Revision note 2026-08-18 01:38 +08:00: Recorded the clean minimal-profile pilot and conditional 100-CAD matrix. The result is exactly the pre-registered seventh recovery at 91/100 strict and 88/100 both-valid, with zero regressions and zero worker/protocol failures. Added the two Git-safe snapshot locations and kept the 95/100 release gate plus all downstream representation work closed.
