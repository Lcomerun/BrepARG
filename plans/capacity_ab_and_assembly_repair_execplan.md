...
- [ ] Let the immutable launcher finish all four formal tasks serially (completed: both VQ-8192 seeds through epoch 99 with formal validation PASS; running: RVQ seed 3; remaining: RVQ seed 4).
...
- [x] (2026-08-17 16:54 +08:00) Snapshotted the completed guarded `directed_trim_local_intersection_topology` combination matrix: 100 unique isolated-worker attempts, 97 STEP-readable, 88 native-valid, 87 strict-valid, 84 both-valid, three restored CADs, zero regressions, and zero worker failures. This is the best no-regression assembly result so far, but the 95/100 GT gate remains closed.
...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root remains healthy: both VQ-8192 seeds have completed 100 epochs and RVQ seed 3 is running. A short-path post-hardening source at `D:\capv5` is dry-run ready but deliberately not started while the immutable older formal run is still active. Baseline parity is exact at 84/100. Conservative local topology repair reaches 85/100 with one restoration and zero regressions, guarded directed trim reaches 86/100 with two restorations and zero regressions, and their guarded combination reaches 87/100 with three restorations and zero regressions. Selective curve rescue and geometric-loop probes did not improve strict validity beyond those three restored CADs. These are valid no-regression partial results, not the 95/100 assembly solution. The plan is not complete until all four capacity runs, unchanged-chain capacity measurements, the remaining independent no-regression repairs, repaired-chain winner measurement, Git-safe snapshots, and remote push are verified.
