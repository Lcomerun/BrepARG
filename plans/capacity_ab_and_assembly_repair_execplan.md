...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root remains healthy: VQ-8192 seed 3 has completed 100 epochs and seed 4 is running. Baseline parity is exact at 84/100. Two independent repair primitives now pass full-cohort no-regression: local topology reaches 85/100 by restoring one self-intersection CAD, and guarded directed trim reaches 86/100 by restoring both wire-build CADs. Neither is the 95/100 assembly solution; their explicit combination and the remaining failure families still require full matrices. The plan is not complete until all four capacity runs, unchanged-chain capacity measurements, the remaining no-regression repairs, repaired-chain winner measurement, Git-safe snapshots, and remote push are verified.
