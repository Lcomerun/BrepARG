...
The face/wire diagnosis milestone is complete and Git-safe. Schema v1 localizes 10 saved-STEP failure CADs to named faces, while schema v2 classifies every one of the 16 self-intersecting wires by exact OCC edge positions and preserves five pre-STEP cases as unavailable. The schema-v2 capacity root is formally complete: all four 100-epoch tasks validate with identical 60k/12k inventories and zero non-finite events. Its Git-safe snapshot preserves all four histories, logs, checkpoint hashes, and five TensorBoard segments, including the legitimate RVQ seed-4 resume segment. The unchanged-chain fixed-cohort result is now conclusive: bypass, VQ-8192, and RVQ reach 70, 69, and 65 strict-valid CADs respectively. VQ-8192 passes the capacity gate with a one-point bypass gap and is selected; RVQ's lower MSE does not translate into better validity and its paired result cannot justify longer sequences. Baseline parity is exact at 84/100. Local topology reaches 85/100, guarded directed trim reaches 86/100, and their explicit combination reaches 87/100, all with zero regressions. The corrected local-pcurve-continuity profile independently reaches 86/100 and adds no recovery beyond guarded directed trim; selective curve rescue changes the three curve-fit failures into STEP-invalid rows but adds no strict-valid CAD beyond the 87/100 combination. The new near-vertex repair explains and restores the one non-unit-solid case without regressing any original success: its signed matrix is 95 STEP-readable, 87 native-valid, 85 strict-valid, and 83 both-valid. It remains an independent primitive rather than a promoted chain because it is still ten CADs below 95/100. The failure-triggered selector correctly preserved its primary routing and produced no worker/protocol faults, but its formal 16-CAD pilot fails at 5 rather than 6 both-valid restores because the remaining candidate changes topology. It is archived as a negative result and does not authorize a 100-CAD selector run. The assembly release gate remains closed at eight CADs below 95/100 when measured against the best explicit no-regression composition, so boundary consistency, sequence regeneration, and AR remain blocked pending a different diagnosed repair or a formal gate decision. Capacity selection and its Git-safe evidence are complete; the plan remains open only for the assembly-chain acceptance path, repaired-chain winner measurement, and downstream release decision.
