...
- [x] (2026-08-14 11:50 +08:00) Separated the bounded precision-probe sampling contract from formal Protocol V5 sampling. A probe now stops after its requested 128 train and 128 validation patches instead of scanning the full split to satisfy parent balancing, pre-cap deduplication, exact-cap, and 90-percent parent-coverage gates. Formal runs retain all four gates unchanged.
...
- [x] (2026-08-14 11:50 +08:00) Re-ran the final implementation regressions after wiring inventory and lifecycle evidence into the 100-CAD coordinator: 131 P0-B/inventory/measurement tests and 91 assembly/V5/V6/sampling tests pass, for 222 passing tests. Python compilation and `git diff --check` also pass.
...
- Observation: The first fp32 probe inherited the formal sampler and therefore appeared stalled at zero GPU utilization before its first batch.
  Evidence: Although the probe requested only 128 patches, parent balancing, deduplication before cap, exact-cap enforcement, and 90-percent split-parent coverage forced a full Protocol V5 source scan. The obsolete local directory `p0b_precision_probe_fp32_20260814` contains no valid precision result and must not be reused.
...
P0-B remains active, but its implementation phase is complete. The formal training loop, immutable four-task launcher, lifecycle-cadenced finite-state audit, exact inventory binding, full-state resume contract, output-root writer lock, fail-closed validator, Windows training-window audit, and learned-VQ 100-CAD measurement coordinator are implemented and covered by 222 final implementation regressions. The finite-state optimization removes complete state scans from the batch hot path while retaining immediate loss/gradient fuses and complete startup, post-resume, epoch-end, and pre-save evidence. The failed first probe was diagnosed as a sampler-contract problem rather than a CUDA failure; it is invalid evidence and will be replaced in new output directories. The remaining P0-B work is runtime evidence: bounded CUDA precision probes, a real rolling-save-to-next-epoch resume smoke, four 100-epoch 60k/12k runs, and the selected healthy learned-VQ fixed-cohort assembly measurement. Boundary consistency therefore remains blocked until those runtime gates pass.
