...
- [ ] Archive and push the four formal P0-B histories, stdout/stderr summaries, TensorBoard events, compact statistics, and checkpoint size/SHA-256 bindings together with a Git-safe P0-A per-case and per-attempt evidence package.
- [ ] Reconstruct and assemble the frozen 100 CADs with both the learned-VQ seed-3 best checkpoint and continuous-bypass seed-3 best checkpoint. Report STEP readability, project strict validity, OpenCascade-native validity, and their conjunction using all 100 attempts per arm.
- [ ] Publish the five-column strict comparison `GT 84% | bypass@300k 70% | FSQ@300k 49% | bypass@60k | VQ@60k`, plus `Delta_q = bypass@60k - VQ@60k` and `Delta_r = GT - bypass@60k` in percentage points.
...
P0-B training is complete. The four bf16 60k/12k tasks reached 100 epochs with zero non-finite events and exact common inventories. Learned VQ retained 3,988 and 4,032 of 4,096 validation codes at seeds 3 and 4, while its lowest parent-cluster curved MSE remained about 2.45 times the continuous-bypass mean. The remaining P0-B work is to archive the formal evidence and run paired seed-3 learned-VQ and bypass checkpoints through the frozen 100-CAD assembly chain. Boundary consistency remains blocked until that paired runtime gate produces complete attempts-based metrics.
